...
async function doctor(): Promise<void> {
  ...
  if (!keeper) bad("KEEPER_PRIVATE_KEY not set");
    ...
    if (bal === 0n) bad("Keeper holds no PLS for gas");
    else ok(`Gas balance ${formatEther(bal)} PLS`);
  }

  if (CFG.treasury === "0x0000000000000000000000000000000000000000")
    bad("TREASURY is the zero address, your 0.15% would be burned");
  ...
  try {
    const amounts: bigint[] = await routerRead.getAmountsOut(
      10n ** 18n, [CFG.wpls, "0x95B303987A60C71504D99Aa1b13B4DA07b0790ab"]);
    ok(`Router quotes fine, 1 PLS is ${formatEther(amounts[1]!)} PLSX`);
