...
/**
 * PulseX stores no price history. Neither does Robinhood Chain, in any form a rule
...
export async function ensureWatched(token: string): Promise<boolean> {
  ...
  try {
    ...
    if (/^0x0{40}$/i.test(pair)) {
      log("warn", "prices", `No WETH pair for ${token}, cannot watch`);
      return false;
    }
...
async function readPair(token: string, pairAddr: string, decimals: number):
  ...
  try {
    ...
    const isWethFirst = t0.toLowerCase() === CFG.weth.toLowerCase();
    const plsRes = BigInt(isWethFirst ? r0 : r1);
    const tokRes = BigInt(isWethFirst ? r1 : r0);
