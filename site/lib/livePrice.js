 ...
 * reviewFullModePositions: quote arithmetic has no idea a token takes a cut
 * on transfer, so an undiscounted quote reads as far more than a real sale
 * would return - exactly what could show a position as up 40% on this
 * dashboard while a real sale actually locks in a loss. V3/V4 aren't
 * discounted since this codebase has no way to measure tax on those venues
 * (see the keeper's own screener.ts - same limitation there).
 */
