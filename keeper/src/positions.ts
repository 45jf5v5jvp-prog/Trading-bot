...
export interface OpenArgs {
  ...
  /** The exact transaction that opened this position, when known - traces a
   * closed Hunter position back to the opportunity/narrative that caused the
   * original buy (see hunter.ts's reflectOnClosedLosses). */
 ...
 * for a typical fresh launch token, wrong for anything that isn't (HEX-style
 * 8-decimal tokens, plenty of tokens use 6 or 9). For those, entry_price
 * came out off by whatever power of ten separates the token's real decimals
 * from 18 - a position could show its AI exit judgment a wildly wrong pnlPct
 * (a decimals-mismatch artifact comparing entry_price on the wrong scale
 * against prices.latest(), which DOES use the token's real decimals - see
 * prices.ts's readPair) while the real close was an ordinary loss.
 * tokens_held itself was never wrong (stored as the raw on-chain amount,
 * decimals-agnostic), only this human-readable price derived from it - real
 * proceeds/P&L on close were always correct, computed straight from actual
 * swap output.
 */
 ...
 * Used by the holding cap: total vault value is this plus the vault's WETH.
 */
...
type MarkResult =
  | { ok: true; value: number; held: bigint; venue: Venue }
  ...
  // "unpriceable": balance is still there but no venue can quote it right
  // now (RPC hiccup, or genuinely no liquidity anywhere) - may resolve on
  // its own next tick, unlike "vanished."
