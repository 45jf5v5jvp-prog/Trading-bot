 ...
 *
 * Field names below keep the "Pls" suffix (suggestedAmountPls etc.) even
 * though this chain's base unit is ETH - same shape-compatibility reasoning
 * as registry.ts/db.ts/positions.ts use throughout this port: the value is
 * denominated in whatever this chain's base unit is, only the name is
 * historical.
 */
...
function describeProfile(p: TokenProfile): string {
  const lines = [
    ...
    `Liquidity: ${Math.round(p.liqPls).toLocaleString()} ETH`,
...
const SYSTEM_PROMPT =
  "You assess Robinhood Chain (Uniswap) tokens for a trading bot's users. Every token you see has " +
...
function verdictTool(maxAmountPls?: number) {
  ...
  if (maxAmountPls !== undefined) {
    properties.suggestedAmountPls = {
      type: "number",
      description:
        `If recommending a buy, how much ETH to actually spend, out of an authorized maximum of ` +
        `${maxAmountPls} ETH. You do not have to use the full amount - spend less than the maximum ` +
...
const EXIT_SYSTEM_PROMPT =
  "You help decide whether to hold or sell an ALREADY-OPEN position in a Robinhood Chain (Uniswap) " +
