...
**Built and tested tonight (23 automated tests, plus a real end-to-end curl
...
- Wallet connect, live vault lookup, live owner/executor/paused/balance reads,
  deposit, withdraw, and a single-rule settings editor (`lib/useVault.js`,
  `pages/index.js`)

**Not built yet** (the rest of the original `web/App.jsx` vision):
multiple bots per vault, charts, the launch-bot toggle UI, CSV export,
pause/close from the UI. The foundation (real data, real auth) is in place for
all of these to be added the same way the pieces above were.
...
## Run it locally first
    ...
    npm run test      # should show 23 passing
