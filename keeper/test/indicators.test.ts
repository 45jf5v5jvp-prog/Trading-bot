...
// candles.ts pulls in db.ts (transitively, config.ts), and RPC_URL has no
// default on this keeper (unlike the PulseChain one) - config.ts requires
// it explicitly. Nothing in this file makes a network call, so any
// well-formed placeholder satisfies it, same convention as portfolio.test.ts.
// Static imports are hoisted above these assignments in ESM, so the two
// modules below are loaded dynamically - same reason ai.test.ts does.
process.env.RPC_URL = process.env.RPC_URL || "https://example-placeholder.invalid";
process.env.DB_PATH = process.env.DB_PATH || "/tmp/claude-0/-home-user-Trading-bot/3123e22c-cef8-5b8c-874f-df7176e2f79c/scratchpad/robinhood-indicators-test.db";

const { toCandles } = await import("../src/candles.js");
const { rsi, macd, bollinger, snapshot, liquidityDropIsSuspicious } = await import("../src/indicators.js");
