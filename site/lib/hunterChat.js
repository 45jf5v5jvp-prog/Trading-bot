 ...
 * old standalone Ask Icaria form, which had its own dedicated address
 * field. Used by the API route to decide whether to run buildProfile
 * before replying. */
const TOKEN_ADDR_RE = /0x[0-9a-fA-F]{40}/;

/**
 * Formats the same mechanical profile the old standalone Ask Icaria showed
 * into plain lines for the model's context - see lib/askIcaria.js's
 * buildProfile for where these numbers actually come from. Robinhood Chain
 * is multi-venue (V2/V3/V4), so unlike PulseChain's Ask Icaria this has no
 * price-history/technical read on an arbitrary token, only the mechanical
 * honeypot/tax/LP-lock/owner checks - describes that plainly rather than
 * fabricating a technical picture that isn't there. Never lets the model
 * guess at a number it wasn't given: an unavailable profile is described as
...
function describeTokenProfile(token, profile) {
  ...
  const lines = [`\nThe owner asked about a specific token, ${profile.symbol} (${token}), traded on ${profile.venue}:`];
  lines.push(
  ...
  lines.push("- No price history/technical read is available for an arbitrary token here - say that directly rather than guessing at momentum.");
  return lines.join("\n");
}
...
const SYSTEM_PROMPT =
  ...
  "that specific token's real mechanical profile below - answer their question about it using " +
  "only those numbers, same as you would about your own trades, and say plainly if it's not " +
  "something you'd buy. You cannot buy anything from this chat right now, so if asked to buy a " +
  "specific token here, say you can't do that yet from a message, not to explain the settings " +
  "elsewhere. You are not certain of anything about the market - be honest about that. Answer " +
  "in a few sentences, conversationally, like a text message, not a report.";
