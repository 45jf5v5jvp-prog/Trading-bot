...
// anything that pulls in config/db. RPC_URL has no default (unlike the
// PulseChain keeper) since there is no verified public mainnet RPC baked
// into this codebase - config.ts requires it explicitly. This test never
// makes a real network call, so any well-formed placeholder satisfies it.
